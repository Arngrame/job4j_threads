...
### ForkJoinPool
Это пул потоков, оптимизированный для выполнения задач, которые можно рекурсивно разбивать на подзадачи.

Особенности:
- использует Work-Stealing алгоритм:
  - каждый поток имеет свою очередь задач DEQUE (двусторонняя очередь);
  - если поток выполнил свои задачи, то он ворует задачи у других потоков из хвоста их очереди => конкуренция за задачи очень низкая;
  - обеспечение эффективного распределения нагрузки (решает проблему балансировки нагрузки, см. предыдущий пункт);
  - поток в первую очередь выполняет СВОИ задачи, что хорошо сказывает на использовании кэша процессора;
  - поток "вор" помнит у кого успешно украл задачу и следующий steal будет проверять с места предыдущей кражи.
- минимизирует накладные расходы на создание задач;
- эффективно использует ядра процессора
- для создания задач используются абстрактные классы RecursiveAction (void) и RecursiveTask (T);
- оба класса создают задачи, которые могут быть рекурсивно разделены на малые подзадачи;
- методы:
  - fork - асинхронный запуск подзадачи в пуле;
  - join - ожидает завершения задачи (RecursiveAction) и возвращает результат (RecursiveTask);
  - invokeAll - запускает несколько подзадач
- оптимальное использование:
  - деление задач на примерно равные части;
  - не создавать много мелких задач;
  - не использовать для блокирующих операций;
  - не использовать для IO операций;
  - ForkJoinPool#commonPool лучше, чем создание нового пула
